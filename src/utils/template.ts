...
export function mapTemplateGroupIdsToRegions(templateGroupData: TemplateGroupByCountry[], regionData: Region[]) {
  ...
  templateGroupData.forEach(({ templateGroupId, countryCode, languageCode }) => {
...
export function filterRegionsAndCountriesByTemplateGroupIds(regionCountry: Region[], templateGroupIds: string[]) {
  ...
  return regionCountry
    .map((region) => {
      ...
      const filteredCountries = region.countries.filter((country) => templateGroupIdSet.has(country.templateGroupId));
