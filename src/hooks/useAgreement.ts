...
export const useAgreement = () => {
  ...
  const getCurrentAgreement = useCallback(
    (url: string) => {
      ...
      const tempLocale = locale === "zh_CN" ? "zh_CN" : ("en_US" as keyof typeof agreement);
  ...
  const goToAgreementPage = useCallback(
    async (url: string) => {
      ...
      window.open(
        `${window.location.origin}${import.meta.env.VITE_APP_BASE_API}/regionWeb/webTermsInfo/getTextById/${id}`,
        "_blank",
        "noopener noreferrer",
      );
    },
